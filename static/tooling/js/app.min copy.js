        ...
        d = {
            ...
            bindEvents: function (b) {
                ...
                a(window).on("resize", function () {
                    ...
                    // if(isok){
                        ...
                        $('#hiddenCode').html(codehtml)
                    // }
                    let active_id = $('#hiddenCode').attr('cur-id')
                    if(active_id != undefined){
                        let target = `select[name="im[${active_id}][shape]"]`
                    ...
                    h.is("button") ? a(document).on("click", g.selector, function () {
                        ...
                        var f = a(this),
                            ...
                            if(f.attr('type') == 'radio'){
                                let target = `select[name="im[${h[0]}][shape]"]`
                                a('#hiddenCode').attr('cur-id',h[0])
                                select_val = a(target).val()
                                if(select_val == null){
                                    a(target).val('rect')
                                }else{
                                    a(target).val(select_val)
                                }
                            }
            ...
            asHTML: function (b) {
                var d = a("<img />"),
                    ...
                    var hashstr =`${sec}-${min}-${hr}`
                    var strIdx = `image-map-${hashstr}`

                c[b].options.file.length > 0 && (e = c[b].options.file), d.attr("src", "#"), d.attr("usemap", `#${strIdx}`);
                var f = a("<map />");
                f.attr("name", `${strIdx}`);
                var g = [];
                a.each(c[b].area, function (d, e) {
                    ...
                    var i = a("<area />");
                    i.attr("data-index", d),
                        i.attr("target", c[b].area[d].target), i.attr("alt", c[b].area[d].title), i.attr("title", c[b].area[d].title), i.attr("href", c[b].area[d].href), i.attr("coords", f.join(",")), i.attr("shape", c[b].area[d].shape == 0 ? 'rect' : c[b].area[d].shape), g.push(i[0].outerHTML)
                }), f.append("\n    " + g.join("\n    ") + "\n");
                return "\n" + d[0].outerHTML + "\n\n" + f[0].outerHTML
            }
        };
