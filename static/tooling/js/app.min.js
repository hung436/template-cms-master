...
! function (a) {
        ...
        d = {
            defaults: {
                ...
                input: {
                    ...
                    // Khởi tạo thêm 1 thuộc tính 
                    linkpage: {
                        selector: 'select[name^="im["][name$="][linkpage]"]',
                        fn: function () {
                            return a(this).attr("name").match(/im\[([\d]+)\]\[([^\)]+)\]/).slice(1)
                        }
                    },
            ...
            init: function (e, f) {
                let initJmap = localStorage.getItem("initJmap")
                let jmap = initJmap != null ? JSON.parse(initJmap) : null
                console.log(jmap)
                var g = this,
                    h = a(this),
                    f = f || {};
                ...
                if (!e) {
                    f = a.extend(!0, d.defaults, f), c[b] = {
                        ...
                        d.bindValues(e)
                        if(location.pathname == '/edit-trang-bao'){
                            if(jmap != null){
                                c[e].area = jmap.area
                                c[e].state = jmap.state
                                "function" == typeof c[e].options.event.init && c[e].options.event.init.apply(g),
                                d.bindValues(e)
                            }
                        }
                ...
                a.each(c[b].options.input, function (f, g) {
                    ...
                    h.is("button") ? a(document).on("click", g.selector, function () {
                        ...
                        var f = a(this),
                            h = g.fn.apply(this),
                            i = f.val();
                            a('#image-mapper-body').find('tr').css('background','none')
                            f.parents('tr').css('background','#17a2b8')
                        let line = f.attr('name').replace(/(im\[\w\])(.*)/gm, `$1`);
                        if (f.attr('name') === `${line}[active]`) {
                            let line = f.attr('name').replace(/(im\[\w\])(.*)/gm, `$1`);
                            console.log(line)
                            let selectActive = $(`select[name='${line}[shape]']`).val()
                            if (selectActive == null) {
                                $(`select[name='${line}[shape]']`).val('rect')
                                $(`select[name='${line}[shape]']`).trigger("change");
                                $(`input[name='${line}[href]']`).prop("disabled", false);
                                $(`input[name='${line}[href]']`).focus();
                            }
                        }
            ...
            renderSVG: function (b) {
                ...
                a.each(c[b].area, function (g, h) {
                    ...
                    "poly" == h.shape ? (i = a(document.createElementNS("http://www.w3.org/2000/svg", "polygon")), i.attr("points", j.join(","))) : "circle" == h.shape ? j.length >= 4 && (i = a(document.createElementNS("http://www.w3.org/2000/svg", "circle")), i.attr("cx", j[0]).attr("cy", j[1]), i.attr("r", Math.sqrt(Math.pow(j[2] - j[0], 2) + Math.pow(j[3] - j[1], 2)))) : j.length >= 4 && (i = a(document.createElementNS("http://www.w3.org/2000/svg", "rect")), i.attr("x", Math.min(j[0], j[2])).attr("y", Math.min(j[1], j[3])), i.attr("width", Math.abs(j[2] - j[0])).attr("height", Math.abs(j[3] - j[1]))), i && (i.attr("class", "image-mapper-shape"), i.attr("data-area-index", g), i.css(c[b].options.style), f.prepend(i), c[b].area[g].el = i)
                })
            },
            ...
            asHTML: function (b) {
                var img = a("<img />"),
                    e = c[b].options.src;
                var strIdx = `image-map`
                c[b].options.file.length > 0 && (e = c[b].options.file), img.attr("src", "#"), img.attr("usemap", `#${strIdx}`);
                var map = a("<map />");
                map.attr("name", `${strIdx}`);
                var arrays = [];
                a.each(c[b].area, function (index, e) {
                    var coordsArr = [];
                    if (a.each(e.coords, function (a, b) {
                        coordsArr.push(b.naturalX, b.naturalY)
                    }), "circle" == e.shape) {
                        var h = Math.round(Math.sqrt(Math.pow(coordsArr[2] - coordsArr[0], 2) + Math.pow(coordsArr[3] - coordsArr[1], 2)));
                        coordsArr = coordsArr.slice(0, 2), coordsArr.push(h)
                    }

                    var elArea = a("<area />");
                    elArea.attr("data-index", index),
                        elArea.attr("linkpage", c[b].area[index].linkpage),
                        elArea.attr("target", c[b].area[index].target),
                        elArea.attr("alt", c[b].area[index].title),
                        elArea.attr("title", c[b].area[index].title),
                        elArea.attr("href", c[b].area[index].href),
                        elArea.attr("coords", coordsArr.join(",")),
                        elArea.attr("shape", c[b].area[index].shape),
                        arrays.push(elArea[0].outerHTML)
                }), map.append("\n    " + arrays.join("\n    ") + "\n");
                return "\n" + img[0].outerHTML + "\n\n" + map[0].outerHTML
            }
        };
    ...
    function (a) {
        a(document).on("init", function () {
           console.log("init")
            let imageMapperRow = a("#image-mapper-table tbody").html();
            ...
            // NEWFUNC
            a(document).on('click', '.togle-fixed', function () {
                $('#image-mapper-table').addClass('table-fixed')
                $(this).hide()
                $('.togle-down').show()
            })
            a(document).on('click', '.togle-down', function () {
                $('#image-mapper-table').removeClass('table-fixed')
                $(this).hide()
                $('.togle-fixed').show()
            })
        })
