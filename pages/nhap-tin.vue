...
export default {
        ...
        AxiosAuth.post(`/api/create-article`, qs.stringify(obj))
          .then(({ data: res }) => {
            if (res.status) {
              ...
              this.$bvToast.toast("Có lỗi trong quá trình đăng tin!", {
                title: `Thông báo`,
                variant: "danger",
                solid: true,
              });
  ...
  watch: {
    content(val) {
      ...
      // let getId = decode(val).replace(/(<div id="(content_title)"(.*)<h1>(.*)<\/h1>)(.*)/sg, `$2`);
      // if(getId == 'content_title'){
      //   let h1 = decode(val).replace(/(<div id="(content_title)"(.*)<h1>(.*)<\/h1>)(.*)/sg, `$4`);
      //   this.title = h1
      // }
      // let chapue = decode(val).replace(/(<div id="(content_caption)"(.*)<h2>(.*)<\/h2>)(.*)/sg, `$2`);
      // if(chapue == 'content_caption'){
      //   let h2 = decode(val).replace(/(<div id="(content_caption)"(.*)<h2>(.*)<\/h2>)(.*)/sg, `$4`);
      //   this.chapue = h2
      // }
      // console.log(this.title, this.chapue)
    },
  },
  head: {
    title: "Nhập tin",
